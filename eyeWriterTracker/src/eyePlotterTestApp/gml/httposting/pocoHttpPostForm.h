...
#include "ofMain.h"
#include "ofURLFileLoader.h"   // using oF’s URL loader API
#include <string>
#include <vector>
...
public:
	...
	void addForm(std::string name, std::string value);
	void submit();                 // synchronous, like before
	void NonThreadedFunction();    // kept for compatibility
	void clearFormData();

	bool wasLastSubmitted();

	int timeoutSeconds;            // preserved

protected:
	void threadedFunction() override; // intentionally no-op (as before)
	...
	bool bLastFormSubmitted = false;
};

#endif // POCOHTTPPOSTFORM_H
