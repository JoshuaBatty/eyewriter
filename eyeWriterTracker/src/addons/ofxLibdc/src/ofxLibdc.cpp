...
void ofxLibdc::grabStill(ofImage& img) {
	...
	img.allocate(width, height, static_cast<ofImageType>(imageType));
	if(imageType == OF_IMAGE_GRAYSCALE) {
		std::memcpy(img.getPixels().getData(), frame->image, width * height);
	} else if(imageType == OF_IMAGE_COLOR) {
		std::memcpy(img.getPixels().getData(), frame->image, width * height * 3);
	}
